...
externalNoInTerminalDAPD :: Applicative f => FilePath -> f DAPDebuggee
externalNoInTerminalDAPD hdbProg = do
    let interpSettings = InterpreterSettings
          { interpreterFlags = mkExternalInterpreterFlags hdbProg
          , interpreterSetup = mkExternalInterpreterFromStdInSetup CreatePipe
          }
    pure $
      DAPDebuggee
        interpSettings
...
externalInTerminalDAPD hdbProg
  ...
  let interpSettings = InterpreterSettings {
    interpreterFlags = mkExternalInterpreterFlags hdbProg
    , interpreterSetup = mkExternalInterpreterFromIOSetup $ extInterpFromTerminalProcess sock
  }
  pure $
    DAPDebuggee
    interpSettings
